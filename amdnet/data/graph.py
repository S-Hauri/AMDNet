...
import numpy as np
from amdnet.utils.general import expand_1st, to_list
from monty.json import MSONable
from amdnet.data import local_env
...
class StructureGraph(MSONable):
    ...
    def graph_to_input(self, graph):
        ...
        ret = [expand_1st(self.atom_converter.convert(graph['atom'])),
        ...
        ret = [np.array(i_, dtype=np.float) for i_ in ret]
    ...
    def get_flat_data(self, graphs, targets=None):
        ...
        for feature in ['atom', 'bond', 'state', 'index1', 'index2']:
            output.append([np.array(x[feature], dtype=np.float) for x in graphs])
...
class HierarchGraphBatchDistanceConvert(BaseGraphBatchGenerator):
    """
    Generate batch of structures with bond distance being expanded using a Expansor
    ...
    def __init__(self,
                 ...
                 batch_size=128,
                 is_shuffle=True,
                 atom_distance_converter=None,
                 motif_distance_converter=None):
        super().__init__(len(a_node_features), targets, batch_size, is_shuffle)
        
        self.atom_distance_converter = atom_distance_converter
        self.motif_distance_converter = motif_distance_converter
        
        self.a_node_features = a_node_features
        self.a_bond_features = a_bond_features
        self.a_state_features = a_state_features
        self.a_index1_list = a_index1_list
        self.a_index2_list = a_index2_list
        
        self.m_node_features = m_node_features
        self.m_bond_features = m_bond_features
        self.m_state_features = m_state_features
        self.m_index1_list = m_index1_list
        self.m_index2_list = m_index2_list

    def process_bond_feature(self, x, is_atom):
        if is_atom:
            return self.atom_distance_converter.convert(x)
        else:
            return self.motif_distance_converter.convert(x)
    ...
    def _generate_inputs(self, batch_index):
        ...
        a_node_list = itemgetter_list(self.a_node_features, batch_index)
        a_bond_list = itemgetter_list(self.a_bond_features, batch_index)
        a_state_list = itemgetter_list(self.a_state_features, batch_index)
        a_index1_list = itemgetter_list(self.a_index1_list, batch_index)
        a_index2_list = itemgetter_list(self.a_index2_list, batch_index)
        
        m_node_list = itemgetter_list(self.m_node_features, batch_index)
        m_bond_list = itemgetter_list(self.m_bond_features, batch_index)
        m_state_list = itemgetter_list(self.m_state_features, batch_index)
        m_index1_list = itemgetter_list(self.m_index1_list, batch_index)
        m_index2_list = itemgetter_list(self.m_index2_list, batch_index)

        return a_node_list, a_bond_list, a_state_list, a_index1_list, a_index2_list, \
               m_node_list, m_bond_list, m_state_list, m_index1_list, m_index2_list


    def _combine_graph_data(self, a_node_list, a_bond_list, a_state_list, a_index1_list, a_index2_list, \
                            m_node_list, m_bond_list, m_state_list, m_index1_list, m_index2_list):
        
        # ATOM part
        # get atom's structure id
        a_gnode = []
        for i, j in enumerate(a_node_list):
            a_gnode += [i] * len(j)
        # get bond features from a batch of structures
        # get bond's structure id
        a_gbond = []
        for i, j in enumerate(a_bond_list):
            a_gbond += [i] * len(j)

        # assemble atom features together
        a_node_list = np.concatenate(a_node_list, axis=0)
        a_node_list = self.process_atom_feature(a_node_list)

        # assemble bond feature together
        a_bond_list = np.concatenate(a_bond_list, axis=0)
        a_bond_list = self.process_bond_feature(a_bond_list, is_atom=True)

        # assemble state feature together
        a_state_list = np.concatenate(a_state_list, axis=0)
        a_state_list = self.process_state_feature(a_state_list)

        # assemble bond indices
        a_index1 = []
        a_index2 = []
        offset_ind = 0
        for ind1, ind2 in zip(a_index1_list, a_index2_list):
            a_index1 += [i + offset_ind for i in ind1]
            a_index2 += [i + offset_ind for i in ind2]
            offset_ind += (max(ind1) + 1)
        
        # MOTIF part
        # get atom's structure id
        m_gnode = []
        for i, j in enumerate(m_node_list):
            m_gnode += [i] * len(j)
        # get bond features from a batch of structures
        # get bond's structure id
        m_gbond = []
        for i, j in enumerate(m_bond_list):
            m_gbond += [i] * len(j)

        # assemble atom features together
        m_node_list = np.concatenate(m_node_list, axis=0)
        m_node_list = self.process_atom_feature(m_node_list)

        # assemble bond feature together
        m_bond_list = np.concatenate(m_bond_list, axis=0)
        m_bond_list = self.process_bond_feature(m_bond_list, is_atom=False)

        # assemble state feature together
        m_state_list = np.concatenate(m_state_list, axis=0)
        m_state_list = self.process_state_feature(m_state_list)

        # assemble bond indices
        m_index1 = []
        m_index2 = []
        offset_ind = 0
        for ind1, ind2 in zip(m_index1_list, m_index2_list):
            m_index1 += [i + offset_ind for i in ind1]
            m_index2 += [i + offset_ind for i in ind2]
            offset_ind += (max(ind1) + 1)

        # Compile the inputs in needed order
        inputs = [expand_1st(a_node_list),
                  expand_1st(a_bond_list),
                  expand_1st(a_state_list),
                  expand_1st(a_index1),
                  expand_1st(a_index2),
                  expand_1st(a_gnode),
                  expand_1st(a_gbond),
                  expand_1st(m_node_list),
                  expand_1st(m_bond_list),
                  expand_1st(m_state_list),
                  expand_1st(m_index1),
                  expand_1st(m_index2),
                  expand_1st(m_gnode),
                  expand_1st(m_gbond)]
