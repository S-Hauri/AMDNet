...
def get_graph_from_schnet_data(data, global_dim=2):
    atom = data['_numbers'].tolist()
    motif_dim = len(atom[0])
    
    bond = np.transpose(data['_offset'])[0].tolist()
    index1 = list(data['_idx_ik'])
    index2 = list(data['_idx_jk'])
    state = [ [0.]*global_dim ]
    graph = {'atom': atom, 'bond': bond, 'index1': index1, 
             'index2': index2, 'state': state}
    return graph, motif_dim